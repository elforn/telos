...
- **`goals`** — `{ [year]: { capstone: Goal[], milestones: Goal[], wow: Goal[], focus: Goal[] } }`. Each `Goal` has a fixed schema:
  ...
  **Type/target: full picker for a new goal, a plain read-only line for an existing one, changed via the ⋮ menu.** Picking a type is the point of creating a goal, so a fresh draft always shows the full interactive pill group + target stepper immediately. Once a goal exists, it's a seldom-touched setting — `goal-dialog` shows a plain, non-interactive `<p>` instead (`#type-readout`, e.g. "5×/week" or "Percentage"; not a button, no chevron, nothing to tap). "Change type" in the ⋮ menu (`#action-change-type-btn`, hidden for a new goal) reveals the same pill group in the readout's place — reveal-once, no re-collapse control, same idiom as the due-date field's overflow-menu toggle just above it in the same file.
