...
9. **Review what's overdue, due today, or due tomorrow** — the bottom-nav bell opens the Upcoming dialog, a skim view spanning every year and list at once; tapping a row jumps to it in context. Optionally get a daily summary notification for the same three buckets. See **Urgency, reminders & the Upcoming view** below.
...
- **`reflections`** — `{ [year]: { scores: { people?, health?, wealth?, contribution?, wonder? }, comment?: string } }` — one annual report per year, not the earlier planned `annual?/Q1..Q4?` per-quarter sketch (superseded). Each of the 5 aspect scores is 1–5 and independently optional — a partially-rated report is valid, not just a fully-rated one. The aspect list (key + i18n label + descriptive hint, e.g. People's hint renders "Family, friends, romance, social") is centralised in `app/utils/reflection.js` (`REFLECTION_ASPECTS`) — nothing outside that file hardcodes the five aspects, mirroring how `color-palette.js` centralises `COLOR_PALETTE`. The same module exports `aggregateScore(reflection)` — the plain average of whichever scores are set (not the unset ones treated as zero), rounded to 1 decimal, `undefined` if nothing is rated yet; nothing outside `reflection.js` computes this average inline. `merge-strategy.js` treats `reflections` as year-keyed data, local-wins-per-year, same as `accentColors`/`images`.

  Entry point is `year-header.js`'s "⋮" menu (`#year-reflection-btn`, trailing value showing the aggregate once one exists, else a prompt to add one) — opens `<reflection-dialog>` (`app/components/reflection-dialog/`), a UI-tier component. Its content is a single flex-column frame slotted into `modal-dialog`'s own scrollable `.body` (`_lib/`, never edited): a fixed title row ("Year in review" only — the aggregate does *not* live here, see the footer below), then a *separate* inner `.reflection-scroll` div (`flex:1; overflow-y:auto`) holding the rows and the highlights field. Because the frame exactly fills `.body`'s box, `.body` itself never has anything to overflow and so never shows its own scrollbar — the visible scrollbar is `.reflection-scroll`'s own, which naturally starts below the title rather than bleeding up alongside it (an actual sticky-positioned title, tried first, still left the *native* scrollbar spanning the full `.body` height including alongside the pinned title — this nested-scroll-container restructuring was the fix, not any sticky/z-index trick). Below the title, one aspect row per entry — a plain divider between rows (`border-block-start`), not a raised/rounded box: five boxes in a row read too heavy. Each row is label + hint + a 5-star `role="radiogroup"` picker, larger touch targets than a typical icon button; a tap pops **every filled star up to and including the tapped value** with a brief scale animation, not just the one tapped — respects `prefers-reduced-motion`; keyboard arrows move *and* commit the selection, roving tabindex defaults to the first star when nothing's rated yet; empty stars use a slightly thinner SVG stroke than filled ones, which read a bit heavy at this size with the shared icon helper's default. Below the rows, a single free-text field labelled "Highlights" (`reflection.highlights-label`/`-placeholder` — deliberately not "Comment": still the same `comment` field in the stored shape/events/export, only the visible label changed).

  The footer is a **single** `slot="footer"` child spanning the full width (`modal-dialog`'s own footer is `justify-content:flex-end`, which would otherwise push every child together against one end) laid out `space-between`: a **text** visibility toggle far left ("Show"/"Hide" — deliberately not an icon, since every icon elsewhere in this dialog represents state, i.e. the stars, not an action), then a right-hand group holding the **live** aggregate badge (recomputed from the dialog's own in-memory scores on every tap, via `aggregateScore` — hidden until at least one aspect is rated) immediately next to a **Close** button — styled exactly like `goal-dialog`'s own `#close` (plain text, `--color-text-secondary`, no fill), not a filled accent pill: this dialog has no separate save step, so Close shouldn't read as a primary/confirming action.

  Edits commit to the store immediately (a star tap/arrow-move, the highlights field's *blur* — not per-keystroke, or the visibility toggle) via `reflection-score-changed`/`reflection-comment-changed`/`reflection-visibility-changed` events the dialog emits upward; the component itself has zero store knowledge (`aggregateScore` is used here only for the live badge, a pure function over local state, not a store read). `year-header.js` owns the store read/write (it already does this directly for photo/accent-colour/tag-visibility, so this isn't a new pattern) and the **session-undo toast**: it snapshots `reflections[year]` when the dialog opens (from either entry point) and, on close, diffs against the current value — if anything changed, one toast covers the whole editing session with an Undo restoring (or removing, if the year had no prior entry) the snapshot, mirroring `goal-dialog`'s blur-save + `home.toast-goal-saved` flow. One report per year — reopening always edits the existing one, no separate create/edit branching.

  **Known library-level limitations, not fixed:** (1) `modal-dialog`'s `<dialog>` caps at `max-block-size: min(85vh, 600px)` unconditionally (`_lib/modules/modal-dialog/modal-dialog.js`) — no per-instance hook to request a taller sheet when the on-screen keyboard isn't open. (2) `modal-dialog`'s template only exposes a default slot (→ `.body`) and `slot="footer"` — there's no third slot between the drag handle and the body, so a dialog's title can never sit visually "in the handle's row"; the closest an app can get is being the first thing in `.body`, which is what this dialog does. Both would need a small Socle-side change (e.g. a `--modal-max-block-size` variable, or a new `slot="header"` insertion point) — not done here since it requires editing `_lib/`, which app-level sessions never do.

  `showCard` (optional boolean on the reflection, default **shown** when absent) is what the visibility toggle controls — omitted entirely when true (never persisted as `showCard: true`, only ever as `showCard: false`), the same "omit rather than store the default" convention as `Goal.color`, just inverted (absent means shown here, not absent-means-none).

  A second entry point is the **on-page summary card**, `#reflection-card` in `home-page.js`'s own template — deliberately a **plain, normal-flow scrollable-area element**, the first child of `<main>`, right above the Capstone section. No special show/hide-on-scroll behaviour of any kind: it scrolls away with the rest of the page exactly like Capstone/milestones/etc. do, and simply sits there the rest of the time. Rounded on the bottom two corners only (flush on top, where it meets the header). Shows the aggregate and highlights text **in one row** (text CSS-truncated to 4 lines) side by side rather than stacked. `home-page.js` subscribes to `reflections` itself (`this.watch('reflections', ...)`) and computes the aggregate via `aggregateScore` directly — it does not go through year-header.js for this. `[hidden]` gates it off whenever no reflection exists yet for the year, or `showCard === false`. Tapping it calls **`year-header.js`'s public `openReflection()`** method (via `this._header.openReflection()`) — the dialog element, the store commits, and the session-undo toast all still live on `year-header.js`; this card is only a second, differently-placed *entry point* into the same flow the "⋮" menu item also uses.

  Full-bleed and flush against the header took two real attempts each, both worth remembering:
  - **Full-bleed** (cancelling `<main>`'s own `padding-inline`, the same technique `year-header.js`'s `.strip-bar` uses) requires `margin-inline: calc(-1 * var(--page-padding))` **and no explicit `inline-size`/`width`** on the card. It's a flex item of `<main>` (`display:flex; flex-direction:column`), so it stretches (the default `align-self`) to the container's *content* width minus its own margin — a negative margin there genuinely widens the stretched box, the same way a plain block element's `width:auto` would. Setting `inline-size: 100%` explicitly (tried first, shipped once) instead pins the box to exactly `<main>`'s already-inset content width and does **not** grow to compensate — the card rendered 40px narrower than the viewport on one side despite the "full width" CSS being present, until caught by measuring `getBoundingClientRect()` against the actual viewport width (a visual claim wrongly taken as verified from source alone; the earlier written record wasn't).
  - **Flush against the header** (`margin-block-end: var(--space-3)` was tried first — the wrong edge; that only affects the gap *below* the card, toward Capstone) needs `margin-block-start: calc(-1 * var(--space-3))` specifically, because `<main>`'s own `padding-block-start` clears the fixed header *plus* a `--space-3` breathing-room buffer meant for whatever's normally first inside it (Capstone). This card wants zero gap instead, so it cancels just that buffer with a matching negative top margin — landing its own top edge exactly at the header's bottom edge (confirmed via `headerRect.bottom === cardRect.top`, not just a visual glance). `<main>` has no `overflow:hidden`, so rendering into its own padding region this way is safe. The gap toward Capstone below is left to `<main>`'s own `gap: var(--space-2)` rather than the card's own `margin-block-end`, so there's only one place that ever sets it.

  **This on-page element went through three designs before landing here — a cautionary tale about not fighting a fixed-position component's own layout machinery.** (1) First shipped as part of `year-header.js`'s own template, gated on the header's existing `.compact` class (hidden in compact/folded, shown in full) — wrong direction; the ask was the opposite. (2) Rebuilt with its *own* independent, more-sensitive scroll threshold (`CARD_FOLD_AT`/`CARD_UNFOLD_AT`, a `.reflection-folded` class) so it would fold/unfold ahead of the header's own much larger compact thresholds — this repeatedly broke in real testing: toggling it changed `:host`'s own rendered height, which fed into the *same* `ResizeObserver` the header's compact transition already uses (updating `--year-header-height`, resetting `body` padding), and that reflow could nudge `window.scrollY` back across the card's own threshold (native scroll-anchoring compensating for content shifting above the viewport) — refolding/unfolding it again immediately, down to "only flashes visible for an instant." Two increasingly elaborate guards (an elapsed-time debounce, then also suppressing `overflow-anchor` around each toggle) each reduced but never fully eliminated it in real device testing, and a `position:absolute` overlay variant tried in between dodged the resize coupling but reintroduced the *original* problem in a different form (floated over the page content instead of pushing it, so it visually covered the Capstone heading). (3) The fix was architectural, not another guard: move the element itself off of `year-header.js` (`position:fixed`, `:host`-height-driven) entirely and onto `home-page.js` (the actual scrollable document), as a normal child of `<main>` with zero custom scroll-reactive JS. Everything in (2) — the constants, the class, the `ResizeObserver` interaction, both guards — is gone; there was never a bug to guard against once the element stopped living inside the fixed header's own height calculation. If a future session is tempted to add scroll-linked show/hide behaviour to something living inside `year-header.js`, budget real device-testing time for exactly this class of feedback loop, or consider whether the element even needs to be inside the fixed header at all.

  **Not automatic in the year markdown export** — a "Reflection" checkbox in `<export-sheet>` (alongside the existing Metadata/Notes checkboxes), shown only for a year export (`show({ showReflection: true })`, a param `<export-sheet>` otherwise defaults to `false` — list/item exports via `list-detail-page.js` never pass it, so the row stays hidden there and the field is simply irrelevant on that path). Only when checked does `home-page.js`'s `_onYearExportConfirm` read `reflections[year]` and pass it to `exportGoalsMarkdown`'s `reflection` option (a `## Reflection` block: one bullet per rated aspect as `Label: N/5`, then the comment) — like the rest of `export-markdown.js`, the aspect labels there are hardcoded English regardless of locale (the export always ignores `t()`), though the aspect *list/order* still comes from `REFLECTION_ASPECTS` so there's one place that ever changes it.
...
### Urgency, reminders & the Upcoming view

**Due-date urgency** (`app/utils/urgency.js`) buckets a `dueDate` + `active` pair into `none → far → month → week → today → overdue` (`urgencyOf`, `URGENCY_ORDER`; `active` is `percentValue(goal) < 100 && !archived` for goals, `status !== 'done' && status !== 'closed'` for items — an inactive or date-less item is always `none`). `mostUrgent(buckets)` reduces a list of buckets to the single worst one; `urgentCount(buckets)` counts only `today`/`overdue` (what roll-up badges show). `matchesDateBucket(key, dueDate, active)` backs the *Overdue/Week/Month/Later/None* filter pills on home/list-detail/lists-overview, reusing `urgencyOf` so the filter always matches what the calendar marker shows.

**Frequency (pace-based) urgency** (`app/utils/frequency-urgency.js`) is a second, independent urgency source for weekly/monthly goals — set once per goal via `tracking.reminderDays` (see the `tracking` shape above), not derived from a due date at all. `frequencyUrgency(goal, active, todayIso?)` returns `{ bucket, tomorrow }` in the same `'none'|'week'|'today'|'overdue'` vocabulary `urgencyOf` uses (never `'far'`/`'month'`, those are dueDate-only concepts), so callers merge the two via `mostUrgent([urgencyOf(goal.dueDate, active), frequencyUrgencyOf(goal, active)])` — see `goal-item.js` — and a goal's row always reflects whichever signal is currently worse.
- **`reminderDays` is `undefined` by default** — an untouched goal doesn't participate in pace urgency (or the Upcoming view/notification digest) at all. Weekly goals opt in via `goal-dialog`'s reminder-day chip row (shown only when `type === 'weekly'`): either `'any'` (times-per-period mode — no specific days pinned) or a non-empty subset of `WEEKDAYS` (`['mon'..'sun']`, `tracking.js`'s single source for day-key order) for scheduled-days mode. Monthly goals have no day picker at all — every monthly goal is unconditionally times-per-period, no opt-in needed.
- **Times-per-period** (`nxBucket`): `slack = daysLeftInPeriod − entriesStillNeeded`. `slack ≤ 0` → `today` (every remaining day is now required); `slack === 1` → `week` (folds into Tomorrow, see below); otherwise `none`. Never produces `overdue` and has **no achievability cutoff** — it keeps nagging in Today even once mathematically unreachable, since there's no single missed commitment to point to (unlike scheduled-days below), so encouraging partial progress stays worthwhile regardless.
- **Scheduled-days** (`scheduledBucket`, weekly only): missing a scheduled day escalates to `overdue` **the very next day**, and keeps showing as `overdue` on every subsequent day (scheduled or not) as long as hitting `target` by period-end is still mathematically possible — once it isn't, it goes quiet (non-actionable noise otherwise). **Today wins over Overdue**: if today is itself a scheduled day and today's own entry isn't logged yet, the bucket is `today` regardless of any earlier miss this week — checked first, so logging today's entry (or hitting `target` early) also clears a still-outstanding overdue state.
- **Tomorrow** is a targeted one-day-out flag (`{ tomorrow: true }`), not a bucket value — mirrors how `urgencyOf`'s plain `'week'` bucket also covers day 2–7, and Tomorrow is carved out of it separately (see `upcoming.js`'s `isTomorrow`).
- Reaching `overdue` this way sets the exact same `data-urgency="overdue"` on `goal-item` as a lapsed dueDate — the full-row-red escalation below doesn't distinguish the two triggers.

**Overdue visual escalation.** Once a goal or list item's merged urgency is `overdue`, the whole row goes full solid `--color-danger` (not just the small calendar-icon chip every other bucket gets) — `goal-item.js`, `list-item.js`, `lists-page-item.js` all set `data-urgency="overdue"` on `:host` and re-theme text/icons/drag-handle to `--color-text-inverse` for contrast. Goals specifically swap their progress-bar colours too: the filled portion becomes `--color-danger`, the unfilled track becomes `--color-danger-track` (`index.html`, `color-mix(in srgb, var(--color-danger) 72%, var(--color-text-primary))` — themed so text stays legible on it in both light/dark) instead of the normal accent-tinted pairing; the frequency dot-strip and Avoid septagon-strip also re-theme onto `--color-text-inverse` since they don't read legibly against solid red. `today` and `tomorrow` get no full-row treatment — only `overdue` is this loud, so it stays uniquely alarming; `today` keeps the small icon-only badge (`app/utils/urgency-badge.js`, unchanged). The year-accent colour picker (`year-header.js`'s `PALETTE`) has no red swatch — "Stone" (a neutral grey) fills that slot instead, so a red year accent can never be confused with the overdue-red row treatment; years already saved with an old red accent (pre-dating this) keep rendering red until manually repicked.

**The Upcoming view** (`app/utils/upcoming.js`) aggregates goals (every year, every section, `!archived` only) and list items (every list, **including archived ones** — a due date stays meaningful regardless of whether the list itself is tucked away) into `{ overdue, today, tomorrow }`. `collectUpcoming({ goals, lists })` is pure/store-agnostic; a goal's placement merges its dueDate bucket and its frequency-pace bucket to whichever is worse, exactly mirroring `goal-item`'s own row. `upcomingBadgeCount({ overdue, today })` is `overdue.length + today.length` — **tomorrow is deliberately excluded from the count** even though tomorrow's entries still appear in the dialog; overdue and today never share an entry (each resolves to exactly one bucket), so no dedup is needed.
- **Entry point**: a bell button in `bottom-nav.js` (`#bell-btn`/`#bell-badge`), hidden entirely when the count is 0, recomputed on every `goals`/`lists` store change (`_subscribeUpcoming`) and on day-change resume (`onDayChange`, see below).
- **`<upcoming-dialog>`** (`app/components/upcoming-dialog/`) is a UI-tier component (property-in via `open(items)`, event-out `upcoming-row-tap`, zero store knowledge) rendering three sections — Overdue/Today/Tomorrow, matching the notification digest's own grouping — each listing whichever goals and items fall into it together (not split into separate Goals/Items groups). Two-line rows: title, then a muted sublabel (`Goal · 2026 · Capstone` or `List · Admin`). Tap-through only, no inline quick actions.
- **Row tap navigates to context, then flashes it** — never a bare edit dialog floating with no context. `bottom-nav.js`'s `_onUpcomingRowTap` sets `setRuntimeState('pendingFocus', { kind, id })` and calls `navigate()` to the goal's year (`home-page.js`) or the item's list (`list-detail-page.js`); the freshly-mounted destination page watches `pendingFocus`, finds the matching `goal-item`/list `item` element by id once its own rows exist, `scrollIntoView({ block: 'center' })`s it, and applies a brief `.nav-flash` class — then clears `pendingFocus`. Deliberately does **not** clear `pendingFocus` on a no-match (the old, wrong-year page instance is often still mounted and races the new one) so whichever instance actually has the goal/item is the one that consumes the signal.

**Bulk "Clear due dates"** (`list-detail-page.js`, bulk-select `⋮` menu) clears `dueDate` (destructuring it away — `{ dueDate, ...rest }`, never set to `null`) on every selected item at once. Commits immediately via `withUndo` with a plain Undo toast, no confirmation dialog — matches the existing convention for other lossy-but-undoable bulk actions (bulk delete). List items only; goals have no bulk-select surface to hang this off of.

**Day-change resume refresh** (`app/utils/day-change-watcher.js`) — nothing above re-checks "what day is it" on its own; every date-dependent render was previously only as fresh as whatever last triggered it. `onDayChange(el, callback)` wires a per-subscriber "has the calendar day moved on" tracker to the element's own `listen()` (auto-cleanup on disconnect), firing only on `visibilitychange → visible`, not a polling timer. Wired into `bottom-nav.js` (urgency pills + Upcoming bell), `home-page.js`, `list-detail-page.js`, and `lists-page.js` — each re-running the same render a real data change already triggers, so an app left open overnight doesn't keep showing yesterday's urgency state until reloaded or edited.

**Notification digest** (opt-in, off by default) — a day-before/today/overdue summary notification, independent per browser install. Two delivery layers plus a Settings toggle:
- **Foreground** (`<due-date-notifier>`, `app/components/due-date-notifier/`) — a Service-tier component, mounted once in `main.js`. Checks on boot and on every `visibilitychange → visible` resume; fires at most one grouped digest per calendar day (`"3 items need attention"`, body sectioned by Overdue/Due today/Due tomorrow — `app/utils/notification-digest.js`'s `buildDigest`, fed by the same `collectUpcoming` the Upcoming dialog uses). Dedup state (`lastNotifiedDate`) lives in its own IndexedDB database (`telos-notifications`, `app/utils/notification-dedup.js`) — deliberately **not** a main-store key, since a background write racing the page's own `setState()` read-modify-write cycle would be lossy. Never calls `Notification.requestPermission()` itself — only the Settings toggle's click does that (a real user gesture is required); re-reads `notificationsEnabled()`/`Notification.permission` fresh on every check (not gated once at mount) so flipping the toggle on takes effect on the very next check via its public `refresh()` method, no reload needed.
- **Background, Chrome-only, best-effort** (`app/sw-extensions.js`) — a `periodicsync` handler appended onto the generated service worker. Since `dist/sw.js` is registered without `{ type: 'module' }` (`_lib/core/sw-manager/sw-manager.js`, read-only), it's a **classic script with no `import`/`export`** — `sw-extensions.js` is deliberately dependency-free plain JS with its own minimal dueDate-only bucket logic (not a reuse of `frequency-urgency.js`). `utils/build.js` reads it (optional — a scaffolded app with no need for it builds unchanged), appends it after the existing `%%TOKEN%%` templating, and folds its contents into the cache-version hash. Registration (`app/utils/periodic-sync.js`, `registerPeriodicSync`/`unregisterPeriodicSync`) is feature-detected and gated on the `periodic-background-sync` permission, called from the same Settings toggle that enables the foreground layer — there's no separate "is it enabled" check inside the SW handler itself, since the handler simply never fires if never registered. **The browser decides actual firing cadence, not this code** — there is no guaranteed timing, and this can't be meaningfully verified via Playwright (confirmed: headless Chromium hard-locks `Notification.permission` to `'denied'` regardless of `context.grantPermissions()`); needs real on-device confirmation, not just desktop testing, same as `setAppBadge` below.
- **Settings toggle** (`bottom-nav.js`, `#notifications-group`) reflects *actually-granted* permission, not just the stored preference — a browser-level permission revoke outside the app shows up as "Off" next time Settings opens. The preference itself (`app/utils/notification-prefs.js`, `localStorage['telos:notificationsEnabled']`) is device-local, same reasoning as theme/locale/export-reminder — deliberately per-install, not exported/synced, so e.g. testing several installs side by side doesn't require juggling one shared setting.
- **Cold-launch handling**: tapping a notification while the app is fully closed needs to land on the Upcoming dialog. `sw-extensions.js`'s `notificationclick` handler focuses an already-open tab (via `postMessage({ type: 'telos-open-upcoming' })`, read by `main.js`) or opens a new one at `?upcoming=1` (read once by `main.js` on boot, then stripped from the URL so a later reload/share doesn't re-open it unprompted) — both eventually call `bottom-nav.js`'s public `openUpcoming()`.
...
- **A frequency goal's pace-based urgency (`app/utils/frequency-urgency.js`) is separate from its `dueDate` urgency** — always merge both via `mostUrgent([urgencyOf(...), frequencyUrgencyOf(...)])` (see `goal-item.js`) rather than reading one and ignoring the other; a goal can be overdue by pace alone with no `dueDate` set at all.
- **Only `overdue` gets the full-row-red escalation.** `today` and `tomorrow` stay icon/badge-only — don't add a full-row treatment for those, it would make `overdue` no longer uniquely alarming.
- **The notification-enabled preference (`telos:notificationsEnabled`) is localStorage, not a store key** — same reasoning as theme/locale/export-reminder: device-local, deliberately excluded from export/import.
- **`app/sw-extensions.js` is a classic script, not a module** — no `import`/`export`. It can't reuse `frequency-urgency.js`/`upcoming.js`; its own dueDate-only bucket logic is a deliberate, documented duplication, not an oversight.
