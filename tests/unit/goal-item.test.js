...
describe('goal-item — decreasing: rendering', () => {
  ...
  it('an "over" wedge in a past week lives outside .septagon-week.current — the DOM precondition the border-removal CSS relies on', () => {
    // A slip one full week ago, allowance 0 -> "over" in a *history* septagon.
    const el = mount(decreasingGoal([isoDaysFromWeekStart(-7)], 0));
    const weeks = [...el.shadowRoot.querySelectorAll('.septagon-strip .septagon-week')];
    const pastWeekWithOver = weeks.find(w => !w.classList.contains('current') && w.querySelector('path[data-state="over"]'));
    expect(pastWeekWithOver).not.toBeUndefined();
  });
