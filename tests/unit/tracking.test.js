...
describe('tracking — recentDots UI classification', () => {
  ...
  it('a monthly goal produces DOT_WINDOW.monthly dots (3) when the oldest is non-empty', () => {
    const goal = monthly(4, ['2026-06-15', '2026-08-10']); // oldest of the 3 shown (June) + current
    const dots = recentDots(goal, '2026-08-10');
...
describe('tracking — recentDots trims a leading missed streak (display only — see CLAUDE.md)', () => {
  ...
  it.each([
    ...
    ['oxX', 2], // trims the single leading miss, shows xX
    ['ooX', 1], // trims both leading misses — nothing but the current period has data
  ...
  it('an entry inside the scored window but outside the display window still changes percentValue, even though recentDots never shows it', () => {
    // PERIOD_WINDOW.weekly (6) is still wider than DOT_WINDOW.weekly (3).
    // 2026-07-07 falls in the week 5 weeks before TODAY (2026-08-10, a
    // Monday) — inside the 6-week scored window, but outside the 3-week
    // displayed one (which only reaches back to 2026-07-27).
    ...
    // recentDots only ever walks the 3 most recent weeks — 2026-07-07 isn't
