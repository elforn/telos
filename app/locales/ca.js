import { defineStrings } from '../../_lib/core/strings.js';

defineStrings({
  'update-banner.available':     'Actualització disponible',
  'update-banner.reload':        'Recarregar',
  'update-banner.dismiss':       'Ignora',

  'home-page.capstone-section':  'Objectiu principal',
  'home-page.milestone-section': 'Fites a 3 mesos',
  'home-page.wow-section':       'Guau cada 8 setmanes',
  'home-page.focus-section':     'Prioritats actuals',
  'home-page.prev-year':         'Any anterior',
  'home-page.next-year':         'Any següent',
  'home-page.year-progress':     'Progrés anual',

  'goal-dialog.placeholder':            'Què aconseguiràs aquest any?',
  'goal-dialog.notes-placeholder': 'Notes',
  'goal-dialog.copy-notes': 'Copia les notes',
  'goal-dialog.close':           'Tanca',
  'goal-dialog.save-and-close': 'Desa i tanca',
  'goal-dialog.title-new':      'Nou objectiu',
  'goal-dialog.title-edit':     'Edita l\'objectiu',
  'goal-dialog.delete':          'Elimina',
  'goal-dialog.draft-clear':     'Esborra',
  'goal-dialog.draft-revert':    'Reverteix',
  'goal-dialog.draft-undo':      'Desfés',
  'goal-dialog.draft-restore':   'Restaura l\'esborrany',

  'goal-item.delete':            'Elimina',
  'goal-item.add-capstone':      'Afegeix objectiu principal',
  'goal-item.add-milestone':     'Afegeix una fita',
  'goal-item.add-wow':           'Afegeix un moment guau',
  'goal-item.add-focus':         'Afegeix una prioritat',
  'goal-item.drag':              'Arrossega per reordenar',
  'goal-item.duedate-aria':      '{title}, {when}',
  'goal-item.freq-aria-weekly':   '{title}, {count} de {target} aquesta setmana',
  'goal-item.freq-aria-monthly':  '{title}, {count} de {target} aquest mes',
  'goal-item.freq-logged-suffix': ', registrat avui',
  'goal-item.freq-history-weekly':  ', setmanes recents: {history}',
  'goal-item.freq-history-monthly': ', mesos recents: {history}',
  'goal-item.freq-history-met':     'assolit',
  'goal-item.freq-history-partial': 'parcial',
  'goal-item.freq-history-missed':  'perdut',
  'goal-item.decr-aria':          '{title}, {pct}% net, {count} faltes aquesta setmana, {target} permeses',
  'goal-item.decr-logged-suffix': ', falta avui',
  'goal-item.decr-history':        ', setmanes recents: {history}',
  'goal-item.decr-history-clean':  'net',
  'goal-item.decr-history-slipped': '{count} faltes',
  'goal-item.countdown-aria':     '{title}, {pct}% complet, {days} dies restants',
  'goal-item.countdown-days':     '{count}d',
  'goal-item.countdown-days-unset': '–',
  'goal-item.failed-suffix':      ', fallat',

  // Urgència dels venciments (compartit per objectius, elements, agregats).
  'urgency.far':          'properament',
  'urgency.month':        'venç aquest mes',
  'urgency.week':         'venç aquesta setmana',
  'urgency.tomorrow':     'venç demà',
  'urgency.today':        'venç avui',
  'urgency.overdue':      'endarrerit',
  'urgency.urgent-count': '{n} per avui o endarrerits',

  // Diàleg «Properament» (campana de la barra de navegació)
  'upcoming.open':             'Properament',
  'upcoming.heading':          'Properament',
  'upcoming.close':            'Tanca',
  'upcoming.empty':            'Res pendent',
  'upcoming.section-overdue':  'Endarrerits',
  'upcoming.section-today':    'Avui',
  'upcoming.section-tomorrow': 'Demà',
  'upcoming.sub-goal':         'Objectiu · {year} · {section}',
  'upcoming.sub-item':         'Llista · {list}',
  'upcoming.detail-days-overdue':     '{days} dies endarrerit',
  'upcoming.detail-missed-count':     '{count} pendents',
  'upcoming.detail-missed-days-aria': 'Pendent {days}',
  'upcoming.hidden-link':      '{count} amagats — toca per revisar',
  'hidden-items.heading':      'Elements amagats',
  'hidden-items.subtitle':     'Endarrerits o amb venciment avui, arxivats o en un any o llista amb els marcadors de venciment desactivats.',
  'hidden-items.empty':        'Res amagat',
  'hidden-items.close':        'Tanca',
  'hidden-items.archived-suffix': ' · Arxivat',

  // Filtres per venciment (objectius + elements)
  'filter.date-label':   'Venciment',
  'filter.date-overdue': 'Endarrerit',
  'filter.date-week':    'Setmana',
  'filter.date-month':   'Mes',
  'filter.date-later':   'Més tard',
  'filter.date-none':    'Sense data',
  'filter.deadlines-hidden': 'Marcadors de venciment amagats',

  'home.toast-goal-saved':       'Objectiu desat',
  'home.toast-goal-hidden':      'Objectiu afegit — amagat pel filtre actiu',
  'home.toast-goal-deleted':     'Objectiu eliminat',
  'home.toast-goal-moved':       'Objectiu mogut a {section}',
  'home.toast-goal-copied':      'Objectiu copiat a {section}',
  'home.toast-goal-archived':    'Objectiu arxivat',
  'home.toast-goal-unarchived':  'Objectiu desarxivat',
  'home.toast-item-created':     'Afegit a {name}',
  'home.toast-item-created-many': 'Afegit a {n} llistes',

  'goal-dialog.duedate-toggle':   'Termini',
  'goal-dialog.duedate-clear':    'Esborra el termini',

  'goal-dialog.archive':                  'Arxiva',
  'goal-dialog.unarchive':               'Desarxiva',
  'goal-dialog.more-actions':             'Més accions',
  'goal-dialog.move-to-year':             'Mou a un altre any',
  'goal-dialog.create-list-item':         'Crea un element de llista',
  'goal-dialog.extract-markdown':         'Comparteix en Markdown',
  'goal-dialog.share-goal':               'Comparteix a Telos',
  'goal-dialog.move-year-label':          'Any',
  'goal-dialog.move-section-label':       'Secció',
  'goal-dialog.move-section-capstone':    'Objectiu principal',
  'goal-dialog.move-section-milestones':  'Fites',
  'goal-dialog.move-section-wow':         'Moments wow',
  'goal-dialog.move-section-focus':       'Focus',
  'goal-dialog.move-section-current':     'Actual',
  'goal-dialog.move-cta':                 'Mou',
  'goal-dialog.copy-cta':                 'Copia',
  'goal-dialog.picker-back':              'Enrere',

  'goal-dialog.type-label':               'Tipus',
  'goal-dialog.type-percentage':          'Percent',
  'goal-dialog.type-weekly':              'Setmanal',
  'goal-dialog.type-monthly':             'Mensual',
  'goal-dialog.type-decreasing':          'Evita',
  'goal-dialog.type-countdown':           'Fins la data',
  'goal-dialog.type-summary-weekly':            '{target}×/set.',
  'goal-dialog.type-summary-monthly':           '{target}×/mes',
  'goal-dialog.type-summary-decreasing-week':   'Evita, {target} falta/set. permesa',
  'goal-dialog.type-summary-decreasing-4weeks': 'Evita, {target} falta/4 set. permesa',
  'goal-dialog.type-summary-countdown':         'Fins la data',
  'goal-dialog.change-type-menu':         'Canvia el tipus',
  'goal-dialog.target-decrease':          'Redueix l’objectiu',
  'goal-dialog.target-increase':          'Augmenta l’objectiu',
  'goal-dialog.target-label-weekly':      'Vegades per setmana',
  'goal-dialog.target-label-monthly':     'Vegades per mes',
  'goal-dialog.target-label-decreasing': 'Faltes permeses',
  'goal-dialog.allowance-period-week':   'per setmana',
  'goal-dialog.allowance-period-4weeks': 'per 4 setmanes',
  'goal-dialog.start-mode-yearstart':    'Inici de l’any',
  'goal-dialog.start-date-label':        'Data d’inici',
  'goal-dialog.tracking-summary-prefix-percentage':  'P: ',
  'goal-dialog.tracking-summary-prefix-weekly':      'S: ',
  'goal-dialog.tracking-summary-prefix-days':        'D: ',
  'goal-dialog.tracking-summary-prefix-decreasing':  'E: ',
  'goal-dialog.tracking-summary-prefix-countdown':   'F: ',
  'goal-dialog.tracking-summary-percentage':        '{percent}% complet',
  'goal-dialog.tracking-summary-weekly':             '{count} de {target}/setmana al {percent}%',
  'goal-dialog.tracking-summary-monthly':            '{count} de {target}/mes al {percent}%',
  'goal-dialog.tracking-summary-days-suffix':        'al {percent}%',
  'goal-dialog.tracking-summary-decreasing-week':    '{count} de {target} permeses/setmana al {percent}%',
  'goal-dialog.tracking-summary-decreasing-4weeks':  '{count} de {target} permeses/4 setmanes al {percent}%',
  'goal-dialog.tracking-summary-countdown':          '{days}d restants al {percent}%',
  'goal-dialog.fixday-toggle':            'Corregeix un dia',
  'goal-dialog.fixday-logged':            'registrat',
  'goal-dialog.fixday-slipped':           'falta',
  'goal-dialog.dow-sun':                  'dg',
  'goal-dialog.dow-mon':                  'dl',
  'goal-dialog.dow-tue':                  'dt',
  'goal-dialog.dow-wed':                  'dc',
  'goal-dialog.dow-thu':                  'dj',
  'goal-dialog.dow-fri':                  'dv',
  'goal-dialog.dow-sat':                  'ds',
  'day-strip.state-missed':               'faltat',
  'day-strip.state-success':              'registrat',
  'day-strip.state-unscheduled':          'registrat, no programat',
  'day-strip.state-pending':              'programat',
  'day-strip.state-blank':                'no programat',
  'goal-dialog.reminder-days-label':      'Dies programats',
  'goal-dialog.reminder-day-mon':         'dl',
  'goal-dialog.reminder-day-tue':         'dt',
  'goal-dialog.reminder-day-wed':         'dc',
  'goal-dialog.reminder-day-thu':         'dj',
  'goal-dialog.reminder-day-fri':         'dv',
  'goal-dialog.reminder-day-sat':         'ds',
  'goal-dialog.reminder-day-sun':         'dg',
  'goal-dialog.reminder-any':             'x',
  'goal-dialog.month-jan':                'gen.',
  'goal-dialog.month-feb':                'febr.',
  'goal-dialog.month-mar':                'març',
  'goal-dialog.month-apr':                'abr.',
  'goal-dialog.month-may':                'maig',
  'goal-dialog.month-jun':                'juny',
  'goal-dialog.month-jul':                'jul.',
  'goal-dialog.month-aug':                'ag.',
  'goal-dialog.month-sep':                'set.',
  'goal-dialog.month-oct':                'oct.',
  'goal-dialog.month-nov':                'nov.',
  'goal-dialog.month-dec':                'des.',

  'year-header.menu':            'Menú',
  'year-header.year-section':    'Aquest any',
  'year-header.app-section':     'Aplicació',
  'year-header.language':        'Idioma',
  'year-header.version':         'Versió',
  'year-header.photo':           'Foto de l\'any',
  'year-header.photo-add':       'Afegeix una foto',
  'year-header.photo-change':    'Canvia la foto',
  'year-header.photo-remove':    'Elimina la foto',
  'year-header.color':           'Color de l\'any',
  'year-header.color-reset':     'Restableix',
  'year-header.color-sky-blue':  'Blau cel',
  'year-header.color-blue':      'Blau',
  'year-header.color-indigo':    'Índigo',
  'year-header.color-violet':    'Violeta',
  'year-header.color-pink':      'Rosa',
  'year-header.color-stone':     'Pedra',
  'year-header.color-orange':    'Taronja',
  'year-header.color-yellow':    'Groc',
  'year-header.color-green':     'Verd',
  'year-header.color-teal':      'Verd blavós',
  'year-header.theme':           'Aparença',
  'year-header.theme-light':     'Clar',
  'year-header.theme-dark':      'Fosc',
  'year-header.theme-system':    'Sistema',
  'year-header.extract-markdown': 'Comparteix en Markdown',
  'year-header.share-year':       'Comparteix a Telos',
  'year-header.change-year':         'Canvia d\'any ({year})',
  'year-header.year-picker-heading': 'Vés a un any',
  'year-header.jump-to-today':       'Avui',
  'year-header.reflection':          'Balanç anual',
  'year-header.reflection-add':     'Afegeix',
  'year-header.reflection-score':   '★ {score}',
  'year-header.reflection-saved':   'Balanç desat',

  'reflection.dialog-heading':      "Balanç de l'any",
  'reflection.highlights-label':       'Moments destacats',
  'reflection.highlights-placeholder': 'Què ha marcat aquest any…',
  'reflection.star-aria':           '{n} de 5 estrelles',
  'reflection.close':               'Tanca',
  'reflection.show':                'Mostra',
  'reflection.hide':                'Amaga',
  'reflection.aspect-people-label':       'Relacions',
  'reflection.aspect-people-hint':        'Família, amics, parella, vida social',
  'reflection.aspect-people-abbr':        'Re',
  'reflection.aspect-health-label':       'Salut',
  'reflection.aspect-health-hint':        'Física, mental, emocional, ambiental, espiritual',
  'reflection.aspect-health-abbr':        'Sa',
  'reflection.aspect-wealth-label':       'Finances',
  'reflection.aspect-wealth-hint':        'Actual, futura, planificada, seguretat, llibertat',
  'reflection.aspect-wealth-abbr':        'Fi',
  'reflection.aspect-contribution-label': 'Contribució',
  'reflection.aspect-contribution-hint':  'Feina, voluntariat, creació, mecenatge, ensenyament, innovació',
  'reflection.aspect-contribution-abbr':  'Co',
  'reflection.aspect-wonder-label':       'Meravella',
  'reflection.aspect-wonder-hint':        'Diversió, exploració, creativitat, aventura, aprenentatge, interessos',
  'reflection.aspect-wonder-abbr':        'Me',
  'reflection.card-aspect-summary':       '{label} {value}',

  'not-found.heading':           'Res aquí',
  'not-found.body':              'Aquesta pàgina no existeix — però els teus objectius t\'esperen.',
  'not-found.cta':               'Anar a aquest any',

  'sync.export-all':             'Exporta les dades',
  'sync.import':                 'Importa',
  'sync.import-preview':         'Aquest fitxer conté {description}.',
  'sync.import-preview-empty':   'Aquest fitxer no té objectius ni elements de llista.',
  'sync.import-preview-log':     'Aquest fitxer conté {count} esdeveniments.',
  'sync.preview-goals-one':      '{n} objectiu ({years})',
  'sync.preview-goals-many':     '{n} objectius ({years})',
  'sync.preview-lists-one':      '{n} llista amb {items}',
  'sync.preview-lists-many':     '{n} llistes amb {items}',
  'sync.preview-items-one':      '{n} element',
  'sync.preview-items-many':     '{n} elements',
  'sync.preview-and':            ' i ',
  'sync.import-cancel':          'Cancel·la',
  'sync.import-merge':           'Fusiona',
  'sync.import-replace':         'Substitueix tot',
  'sync.import-sure':            'Segur?',
  'sync.import-sure-aria':       'Confirma: substitueix totes les dades',
  'sync.import-confirm':         'Afegits {goals} objectius i {items} elements de llista.',
  'sync.import-replace-confirm': 'Totes les dades han estat substituïdes.',
  'sync.import-close':           'Tanca',
  'sync.import-error':           'Fitxer d\'exportació invàlid o incompatible.',
  'sync.handoff-goal-year-label':    'Any',
  'sync.handoff-goal-section-label': 'Secció',
  'sync.handoff-goal-message':       'Han compartit "{title}" amb tu. Tria on afegir-lo:',
  'sync.handoff-goal-confirm':       'S\'ha afegit "{title}".',
  'sync.handoff-item-confirm':       'S\'ha afegit "{title}".',
  'sync.handoff-items-confirm':      'S\'han afegit {count} elements.',
  'sync.handoff-add-to-list':        'Afegeix a una llista',

  'share.error': 'No s\'ha pogut compartir. Torna-ho a provar.',

  'lists-page.heading':   'Llistes',
  'lists-page.add':       'Nova llista',
  'lists-page.drag':     'Arrossega per reordenar',
  'lists-page.menu':           'Menú',
  'lists-page.rollup-toggle':  'Indicadors de data',

  'list-detail.back':        'Llistes',
  'list-detail.add':         'Nou item',
  'list-detail.menu':        'Opcions',
  'list-detail.edit-name':   'Edita el nom de la llista',
  'list-detail.delete-list':         'Elimina la llista',
  'list-detail.status-label': 'Estat',
  'list-detail.status-show':        'Mostra',
  'list-detail.status-hide':        'Amaga',
  'list-detail.cancel-selection':   'Cancel·la',
  'list-detail.selection-count':    '{n} elements',
  'list-detail.bulk-delete':        'Elimina',
  'list-detail.bulk-status':        'Estat',
  'list-detail.bulk-move':          'Mou',
  'list-detail.bulk-clear-dates':   'Esborra dates de venciment',
  'list-detail.bulk-tags':          'Etiquetes',
  'list-detail.bulk-more':          'Més',
  'list-detail.bulk-status-label':  'Canvia l\'estat',
  'list-detail.bulk-tags-label':    'Afegeix etiquetes',
  'list-detail.bulk-tag-apply-all': 'Aplica {tag} a tots',
  'list-detail.extract-markdown':      'Comparteix en Markdown',
  'list-detail.share-list':            'Comparteix a Telos',
  'list-detail.archive-btn':             'Arxiva',
  'list-detail.unarchive-btn':           'Desarxiva',
  'list-detail.bulk-extract-markdown': 'Comparteix en Markdown',
  'list-detail.bulk-share':            'Comparteix a Telos',
  'list-detail.bulk-delete-toast':  '{n} elements eliminats',
  'list-detail.bulk-clear-dates-toast': 'Data de venciment esborrada de {n} elements',
  'list-detail.bulk-status-toast':  '{n} elements actualitzats',
  'list-detail.add-from-text':       'Afegir des de text',
  'list-detail.import-toast':        '{n} elements importats',

  'import-text.heading':             'Importar elements',
  'import-text.placeholder':         'Un element per línia. Sagna una línia per convertir-la en nota.',
  'import-text.count':               '{n} elements',
  'import-text.count-extras':        '{n} elements · {m} amb notes o enllaços',
  'import-text.cancel':              'Cancel·la',
  'import-text.cta':                 'Importar',
  'import-text.draft-clear':         'Esborra',
  'import-text.draft-undo':          'Desfés',

  'list-dialog.name-placeholder': 'Nom de la llista',
  'list-dialog.close':            'Tanca',
  'list-dialog.save-and-close':   'Desa i tanca',
  'list-dialog.title-new':        'Nova llista',
  'list-dialog.title-edit':       'Edita la llista',
  'list-dialog.delete':           'Elimina',
  'list-dialog.draft-clear':      'Esborra',
  'list-dialog.draft-revert':     'Reverteix',
  'list-dialog.draft-undo':       'Desfés',
  'list-dialog.draft-restore':    'Restaura l\'esborrany',

  'item-dialog.title-placeholder':'Títol',
  'item-dialog.status-label':     'Estat',
  'item-dialog.status-open':      'Obert',
  'item-dialog.status-paused':    'En pausa',
  'item-dialog.status-done':      'Fet',
  'item-dialog.status-closed':    'Tancat',
  'item-dialog.note-placeholder': 'Nota',
  'item-dialog.copy-note': 'Copia la nota',
  'item-dialog.url-placeholder':  'Enllaç (https://…)',
  'item-dialog.url-toggle':       'Enllaç',
  'item-dialog.url-open':         'Obre',
  'item-dialog.duedate-toggle':   'Data límit',
  'item-dialog.duedate-clear':    'Esborra la data límit',
  'item-dialog.close':            'Tanca',
  'item-dialog.save-and-close':   'Desa i tanca',
  'item-dialog.title-new':        'Nou element',
  'item-dialog.title-edit':       'Edita l\'element',
  'item-dialog.delete':           'Elimina',
  'item-dialog.draft-clear':      'Esborra',
  'item-dialog.draft-revert':     'Reverteix',
  'item-dialog.draft-undo':       'Desfés',
  'item-dialog.draft-restore':    'Restaura l\'esborrany',

  'item-dialog.move-to-list':           'Mou a una llista',
  'item-dialog.add-to-goal':            'Afegeix a un objectiu',
  'item-dialog.extract-markdown':       'Comparteix en Markdown',
  'item-dialog.share-item':             'Comparteix a Telos',
  'item-dialog.picker-back':            'Enrere',
  'item-dialog.move-cta':               'Mou',
  'item-dialog.copy-cta':               'Copia',
  'item-dialog.no-other-lists':         'Cap altra llista',
  'item-dialog.new-list-btn':           '＋ Llista nova',
  'item-dialog.new-list-placeholder':   'Nom de la llista',
  'item-dialog.goal-year-label':        'Any',
  'item-dialog.goal-section-label':     'Secció',
  'item-dialog.goal-section-capstone':  'Objectiu principal',
  'item-dialog.goal-section-milestones':'Fites',
  'item-dialog.goal-section-wow':       'Moments wow',
  'item-dialog.goal-section-focus':     'Focus',
  'item-dialog.goal-add-cta':           'Afegeix a un objectiu',
  'item-dialog.goal-already-added':     'Ja afegit',
  'item-dialog.in-goals-label':         'Als objectius',
  'item-dialog.move-toast':             'Mogut a {name}',
  'item-dialog.move-toast-many':        'Mogut a {n} llistes',
  'item-dialog.copy-toast':             'Copiat a {name}',
  'item-dialog.copy-toast-many':        'Copiat a {n} llistes',
  'item-dialog.promote-toast':          'Afegit a {year} {section}',
  'item-dialog.tags-label':             'Etiquetes',
  'item-dialog.tags-placeholder':       'Afegeix etiqueta…',
  'item-dialog.more-actions':           'Més accions',
  'item-dialog.picker-selected':        '{n} seleccionat(s)',
  'item-dialog.picker-clear':           'Esborra la selecció',

  'list-item.delete':          'Elimina',
  'list-item.drag':      'Arrossega per reordenar',
  'list-item.duedate-aria': '{title}, {when}',

  'lists.toast-list-saved':      'Llista desada',
  'lists.toast-list-hidden':     'Llista afegida — amagada pel filtre actiu',
  'lists.toast-list-deleted':    '«{name}» eliminada',
  'lists.toast-list-archived':   'Llista arxivada',
  'lists.toast-list-unarchived': 'Llista desarxivada',
  'lists.toast-item-saved':   'Item desat',
  'lists.toast-item-hidden':  'Item afegit — amagat pel filtre actiu',
  'lists.toast-item-deleted': 'Item eliminat',

  'list-detail.filter-toggle':     'Filtra',
  'list-detail.filter-search':     'Cerca elements…',
  'list-detail.filter-status-all': 'Tots',
  'list-detail.filter-clear':      'Elimina els filtres',
  'list-detail.filter-expand':     'Més filtres',
  'list-detail.filter-empty':      'Cap element coincideix',
  'list-detail.filter-count':      '{count} elements coincideixen',

  'lists-page.filter-toggle':      'Filtra',
  'lists-page.filter-search':      'Cerca llistes…',
  'lists-page.filter-expand':      'Més filtres',
  'lists-page.filter-clear':       'Elimina els filtres',
  'lists-page.filter-empty':       'Cap llista coincideix',
  'lists-page.filter-empty-only':  'Buides',
  'lists-page.filter-not-empty':   'No buides',
  'lists-page.filter-archived':    'Arxivades',
  'lists-page.filter-count':       '{count} llistes coincideixen',

  'home-page.filter-toggle':       'Filtra',
  'home-page.filter-search':       'Cerca objectius…',
  'home-page.filter-expand':       'Més filtres',
  'home-page.filter-done':         'Fet',
  'home-page.filter-ongoing':      'En curs',
  'home-page.filter-not-started':  'Sense iniciar',
  'home-page.filter-archived':     'Arxivats',
  'home-page.filter-clear':        'Elimina els filtres',
  'home-page.filter-empty':        'Cap objectiu coincideix',
  'home-page.filter-count':        '{count} objectius coincideixen',

  'bottom-nav.years':    'Anys',
  'bottom-nav.lists':    'Llistes',
  'bottom-nav.settings': 'Ajustos',

  'settings.repair':           'Reparar la instal·lació',
  'settings.export-reminder':  'Recordatori d\'exportació',
  'settings.reminder-on':      'Mostra',
  'settings.reminder-off':     'Amaga',
  'settings.tag-strip':        'Colors d\'etiquetes',
  'settings.deadlines':        'Marcadors de venciment',
  'settings.toggle-on':        'Activat',
  'settings.toggle-off':       'Desactivat',
  'settings.level-off':        'Desactivat',
  'settings.level-warn':       'Avís',
  'settings.level-full':       'Complet',
  'settings.tags-shown':       'Mostrades',
  'settings.tags-hidden':      'Ocultes',
  'settings.notifications':        'Notificacions',
  'settings.notifications-on':     'Activades',
  'settings.notifications-off':    'Desactivades',
  'settings.notifications-denied': 'Les notificacions estan bloquejades per a aquest lloc — activa-les als ajustos del navegador per activar-ho aquí.',
  'settings.notify-after':      'Notifica després de',
  'settings.notify-anytime':   'En qualsevol moment',
  'settings.notifications-hint': 'Les notificacions només s’activen quan l’aplicació és visible.',

  'notifications.digest-title':    '{count} elements necessiten atenció',
  'notifications.digest-overdue':  'Vençuts ({count})',
  'notifications.digest-today':    'Venciment avui ({count})',
  'notifications.digest-hidden':     '{count} amagats',
  'notifications.digest-hidden-only-title': '{count} amagats',
  'notifications.digest-hidden-only-body':  'Toca per revisar',
  'sync.backup-downloaded':    'Les teves dades s\'han guardat abans de l\'actualització.',
  'sync.backup-before-import': 'Les teves dades s\'han guardat.',

  'export-sheet.metadata': 'Metadades',
  'export-sheet.notes':    'Notes',
  'export-sheet.reflection': 'Balanç anual',
  'export-sheet.extract':  'Comparteix en Markdown',
  'export.copied':         'Copiat al porta-retalls',

  'undo.button':  'Desfés',
  'filter.toast-show': 'Mostra',
  'toast.close':  'Tanca',
  'tag.remove':   'Elimina {tag}',
  'dialog.saved-sr': 'Desat',

  'color-picker.none':   'Cap color',
  'color-picker.red':    'Vermell',
  'color-picker.orange': 'Taronja',
  'color-picker.yellow': 'Groc',
  'color-picker.green':  'Verd',
  'color-picker.teal':   'Verd blavós',
  'color-picker.blue':   'Blau',
  'color-picker.purple': 'Violeta',

  'goal-dialog.back-to-edit':            'Edita',
  'modal-dialog.tab-label':              'Pàgina {index} de {count}',

  'goal-analytics.page-title-score':      'Puntuació',
  'goal-analytics.page-title-activity':   'Activitat',
  'goal-analytics.page-title-streaks':    'Ratxes',

  'goal-analytics.unit-week':            'setmana',
  'goal-analytics.unit-week-plural':     'setmanes',
  'goal-analytics.unit-month':           'mes',
  'goal-analytics.unit-month-plural':    'mesos',

  'goal-analytics.stat-slips':           'relliscades',
  'goal-analytics.stat-updates':         'actualitzacions',
  'goal-analytics.stat-entries':         'entrades',

  'goal-analytics.vs-month':             'vs mes',
  'goal-analytics.vs-quarter':           'vs trimestre',
  'goal-analytics.vs-year':              'vs any',
  'goal-analytics.not-enough-history':   'historial insuficient',
  'goal-analytics.pts':                  'pts',

  'goal-analytics.change-over-time':     'Evolució en el temps',
  'goal-analytics.progress-chart-title': 'Progrés vs ritme previst',
  'goal-analytics.legend-achieved':      'Assolit',
  'goal-analytics.legend-expected':      'Ritme previst',
  'goal-analytics.last-n-periods':       'últims {n} períodes',

  'goal-analytics.pace-insufficient': 'No hi ha prou impuls recent per projectar una data de finalització.',
  'goal-analytics.pace-on-track':     'Al ritme previst — final projectat cap al termini del {deadline}.',
  'goal-analytics.pace-behind':       '≈{monthsLabel} de retard — projectat {projected} amb un termini el {deadline}.',
  'goal-analytics.pace-ahead':        '≈{monthsLabel} d’avantatge — projectat {projected}.',
  'goal-analytics.pace-projected':    'Al ritme actual, ~100 % cap a {projected}.',
  'goal-analytics.month-singular':    '1 mes',
  'goal-analytics.month-plural':      '{n} mesos',

  'goal-analytics.score-contribute':     '{label} contribueixen a',
  'goal-analytics.now':                  'ara',
  'goal-analytics.older-periods-note':   'Les setmanes anteriors apareixeran aquí',

  'goal-analytics.a11y-sparkline': 'Minigràfic del progrés recent',
  'goal-analytics.a11y-progress-chart': 'Gràfic de línies del progrés respecte al ritme previst',
  'goal-analytics.a11y-consistency': 'Gràfic de barres del resultat de cada període respecte a l’objectiu',
  'goal-analytics.a11y-histogram': 'Gràfic de barres del nombre d’entrades per període',
  'goal-analytics.a11y-calendar': 'Mapa de calor dels dies registrats durant l’últim any',
  'goal-analytics.a11y-freqgrid': 'Graella de freqüència d’entrades per dia de la setmana i mes',
  'goal-analytics.a11y-score-grid': 'Graella de resultats per període que contribueixen a la puntuació',
  'goal-analytics.page-title-overview': 'Resum',
  'goal-analytics.consistency-title': 'Regularitat',
  'goal-analytics.consistency-note-avg': 'Cada barra és la mitjana dels períodes que conté, cadascun limitat al 100 %.',
  'goal-analytics.quarter-prefix':    'T',
  'goal-analytics.timeframe-label':   'Període',
  'goal-analytics.timeframe-week':    'Setmana',
  'goal-analytics.timeframe-month':   'Mes',
  'goal-analytics.timeframe-quarter': 'Trimestre',
  'goal-analytics.timeframe-year':    'Any',

  'goal-analytics.count-per-timebox': 'Recompte per període',
  'goal-analytics.calendar-title':    'Calendari',
  'goal-analytics.frequency-title':   'Freqüència per dia',
  'goal-analytics.best-streaks':      'Millors ratxes',
  'goal-analytics.no-streaks-yet':    'Encara no hi ha cap ratxa — torna quan hi hagi més historial.',
  'goal-analytics.days-abbrev':       'd',
}, 'ca');
