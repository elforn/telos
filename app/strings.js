import { defineStrings } from '../_lib/core/strings.js';

defineStrings({
  'update-banner.available':  'Update available',
  'update-banner.reload':     'Reload',
  'update-banner.dismiss':    'Dismiss',

  'home-page.capstone-section':     'Capstone',
  'home-page.milestone-section':    '3-Month Milestones',
  'home-page.wow-section':          '8-Week Wow',
  'home-page.focus-section':        'Forward Focus',
  'home-page.prev-year':         'Previous year',
  'home-page.next-year':         'Next year',
  'home-page.year-progress':     'Year progress',

  'goal-dialog.placeholder': 'What will you achieve this year?',
  'goal-dialog.notes-placeholder': 'Notes',
  'goal-dialog.copy-notes': 'Copy notes',
  'goal-dialog.close':           'Close',
  'goal-dialog.save-and-close': 'Save and close',
  'goal-dialog.title-new':      'New goal',
  'goal-dialog.title-edit':     'Edit goal',
  'goal-dialog.delete':         'Delete',
  'goal-dialog.draft-clear':    'Clear',
  'goal-dialog.draft-revert':   'Revert',
  'goal-dialog.draft-undo':     'Undo',
  'goal-dialog.draft-restore':  'Restore draft',

  'goal-item.delete':          'Delete',
  'goal-item.add-capstone':    'Add capstone goal',
  'goal-item.add-milestone':   'Add milestone',
  'goal-item.add-wow':         'Add wow moment',
  'goal-item.add-focus':       'Add to Forward Focus',
  'goal-item.drag':            'Drag to reorder',
  'goal-item.duedate-aria':    '{title}, {when}',
  'goal-item.freq-aria-weekly':   '{title}, {count} of {target} this week',
  'goal-item.freq-aria-monthly':  '{title}, {count} of {target} this month',
  'goal-item.freq-logged-suffix': ', logged today',
  'goal-item.freq-history-weekly':  ', recent weeks: {history}',
  'goal-item.freq-history-monthly': ', recent months: {history}',
  'goal-item.freq-history-met':     'met',
  'goal-item.freq-history-partial': 'partial',
  'goal-item.freq-history-missed':  'missed',
  'goal-item.decr-aria':          '{title}, {pct}% clean, {count} slips this week, {target} allowed',
  'goal-item.decr-logged-suffix': ', slipped today',
  'goal-item.decr-history':        ', recent weeks: {history}',
  'goal-item.decr-history-clean':  'clean',
  'goal-item.decr-history-slipped': '{count} slips',
  'goal-item.countdown-aria':     '{title}, {pct}% complete, {days} days remaining',
  'goal-item.countdown-days':     '{count}d',
  'goal-item.countdown-days-unset': '–',
  'goal-item.failed-suffix':      ', failed',

  // Deadline / due-date urgency phrasing (shared by goals, items, roll-ups).
  'urgency.far':          'upcoming',
  'urgency.month':        'due this month',
  'urgency.week':         'due this week',
  'urgency.tomorrow':     'due tomorrow',
  'urgency.today':        'due today',
  'urgency.overdue':      'overdue',
  'urgency.urgent-count': '{n} due today or overdue',

  // Upcoming dialog (bottom-nav bell) — skims overdue/today/tomorrow goals and
  // list items across every year and list in one place.
  'upcoming.open':             'Upcoming',
  'upcoming.heading':          'Upcoming',
  'upcoming.close':            'Close',
  'upcoming.empty':            'Nothing upcoming',
  'upcoming.section-overdue':  'Overdue',
  'upcoming.section-today':    'Today',
  'upcoming.section-tomorrow': 'Tomorrow',
  'upcoming.sub-goal':         'Goal · {year} · {section}',
  'upcoming.sub-item':         'List · {list}',
  'upcoming.detail-days-overdue':     '{days} days overdue',
  'upcoming.detail-missed-count':     '{count} missing',
  'upcoming.detail-missed-days-aria': 'Missing {days}',
  'upcoming.hidden-link':      '{count} hidden — tap to review',
  'hidden-items.heading':      'Hidden items',
  'hidden-items.subtitle':     'Overdue or due today, either archived or in a year/list with deadline markers turned off.',
  'hidden-items.empty':        'Nothing hidden',
  'hidden-items.close':        'Close',
  'hidden-items.archived-suffix': ' · Archived',

  // Date filter pills (goals + list items)
  'filter.date-label':   'Due date',
  'filter.date-overdue': 'Overdue',
  'filter.date-week':    'Week',
  'filter.date-month':   'Month',
  'filter.date-later':   'Later',
  'filter.date-none':    'None',
  'filter.deadlines-hidden': 'Deadline markers hidden',

  'home.toast-goal-saved':   'Goal saved',
  'home.toast-goal-hidden':  'Goal added — hidden by the current filter',
  'home.toast-goal-deleted': 'Goal deleted',
  'home.toast-goal-moved':         'Goal moved to {section}',
  'home.toast-goal-copied':        'Goal copied to {section}',
  'home.toast-goal-archived':      'Goal archived',
  'home.toast-goal-unarchived':    'Goal unarchived',
  'home.toast-item-created':       'Added to {name}',
  'home.toast-item-created-many':  'Added to {n} lists',

  'goal-dialog.duedate-toggle':          'Deadline',
  'goal-dialog.duedate-clear':           'Clear deadline',

  'goal-dialog.more-actions':            'More actions',
  'goal-dialog.archive':                 'Archive',
  'goal-dialog.unarchive':               'Unarchive',
  'goal-dialog.move-to-year':            'Move to year',
  'goal-dialog.create-list-item':        'Create list item',
  'goal-dialog.extract-markdown':        'Share Markdown',
  'goal-dialog.share-goal':              'Share to Telos',
  'goal-dialog.move-year-label':         'Year',
  'goal-dialog.move-section-label':      'Section',
  'goal-dialog.move-section-capstone':   'Capstone',
  'goal-dialog.move-section-milestones': 'Milestones',
  'goal-dialog.move-section-wow':        'Wow Moments',
  'goal-dialog.move-section-focus':      'Focus',
  'goal-dialog.move-section-current':    'Current',
  'goal-dialog.move-cta':               'Move',
  'goal-dialog.copy-cta':               'Copy',
  'goal-dialog.picker-back':            'Back',

  'goal-dialog.type-label':             'Type',
  'goal-dialog.type-percentage':        'Percent',
  'goal-dialog.type-weekly':            'Weekly',
  'goal-dialog.type-monthly':           'Monthly',
  'goal-dialog.type-decreasing':        'Avoid',
  'goal-dialog.type-countdown':         'To date',
  'goal-dialog.type-summary-weekly':             '{target}×/week',
  'goal-dialog.type-summary-monthly':            '{target}×/month',
  'goal-dialog.type-summary-decreasing-week':    'Avoid, {target} slip/wk allowed',
  'goal-dialog.type-summary-decreasing-4weeks':  'Avoid, {target} slip/4wks allowed',
  'goal-dialog.type-summary-countdown':          'To date',
  'goal-dialog.change-type-menu':       'Change type',
  'goal-dialog.target-decrease':        'Decrease target',
  'goal-dialog.target-increase':        'Increase target',
  'goal-dialog.target-label-weekly':    'Times per week',
  'goal-dialog.target-label-monthly':   'Times per month',
  'goal-dialog.target-label-decreasing': 'Slip-ups allowed',
  'goal-dialog.allowance-period-week':   'per week',
  'goal-dialog.allowance-period-4weeks': 'per 4 weeks',
  'goal-dialog.start-mode-yearstart':    'Year start',
  'goal-dialog.start-date-label':        'Start date',
  'goal-dialog.tracking-summary-prefix-percentage':  'P: ',
  'goal-dialog.tracking-summary-prefix-weekly':      'W: ',
  'goal-dialog.tracking-summary-prefix-days':        'D: ',
  'goal-dialog.tracking-summary-prefix-decreasing':  'A: ',
  'goal-dialog.tracking-summary-prefix-countdown':   'T: ',
  'goal-dialog.tracking-summary-percentage':        '{percent}% complete',
  'goal-dialog.tracking-summary-weekly':             '{count} of {target}/week at {percent}%',
  'goal-dialog.tracking-summary-monthly':            '{count} of {target}/month at {percent}%',
  'goal-dialog.tracking-summary-days-suffix':        'at {percent}%',
  'goal-dialog.tracking-summary-decreasing-week':    '{count} of {target} allowed/week at {percent}%',
  'goal-dialog.tracking-summary-decreasing-4weeks':  '{count} of {target} allowed/4 weeks at {percent}%',
  'goal-dialog.tracking-summary-countdown':          '{days}d left at {percent}%',
  'goal-dialog.fixday-toggle':          'Fix a day',
  'goal-dialog.fixday-logged':          'logged',
  'goal-dialog.fixday-slipped':         'slipped',
  'goal-dialog.dow-sun':                'Sun',
  'goal-dialog.dow-mon':                'Mon',
  'goal-dialog.dow-tue':                'Tue',
  'goal-dialog.dow-wed':                'Wed',
  'goal-dialog.dow-thu':                'Thu',
  'goal-dialog.dow-fri':                'Fri',
  'goal-dialog.dow-sat':                'Sat',
  'day-strip.state-missed':             'missed',
  'day-strip.state-success':            'logged',
  'day-strip.state-unscheduled':        'logged, not scheduled',
  'day-strip.state-pending':            'scheduled',
  'day-strip.state-blank':              'not scheduled',
  'goal-dialog.reminder-days-label':    'Scheduled days',
  'goal-dialog.reminder-day-mon':       'M',
  'goal-dialog.reminder-day-tue':       'T',
  'goal-dialog.reminder-day-wed':       'W',
  'goal-dialog.reminder-day-thu':       'T',
  'goal-dialog.reminder-day-fri':       'F',
  'goal-dialog.reminder-day-sat':       'S',
  'goal-dialog.reminder-day-sun':       'S',
  'goal-dialog.reminder-any':           'x',
  'goal-dialog.month-jan':              'Jan',
  'goal-dialog.month-feb':              'Feb',
  'goal-dialog.month-mar':              'Mar',
  'goal-dialog.month-apr':              'Apr',
  'goal-dialog.month-may':              'May',
  'goal-dialog.month-jun':              'Jun',
  'goal-dialog.month-jul':              'Jul',
  'goal-dialog.month-aug':              'Aug',
  'goal-dialog.month-sep':              'Sep',
  'goal-dialog.month-oct':              'Oct',
  'goal-dialog.month-nov':              'Nov',
  'goal-dialog.month-dec':              'Dec',

  'year-header.menu':         'Menu',
  'year-header.year-section': 'This year',
  'year-header.app-section':  'App',
  'year-header.language':     'Language',
  'year-header.version':      'Version',
  'year-header.photo':        'Year photo',
  'year-header.photo-add':    'Add photo',
  'year-header.photo-change': 'Change photo',
  'year-header.photo-remove': 'Remove photo',
  'year-header.color':           'Year color',
  'year-header.color-reset':     'Reset to default',
  'year-header.color-sky-blue':  'Sky blue',
  'year-header.color-blue':      'Blue',
  'year-header.color-indigo':    'Indigo',
  'year-header.color-violet':    'Violet',
  'year-header.color-pink':      'Pink',
  'year-header.color-stone':     'Stone',
  'year-header.color-orange':    'Orange',
  'year-header.color-yellow':    'Yellow',
  'year-header.color-green':     'Green',
  'year-header.color-teal':      'Teal',
  'year-header.theme':        'Appearance',
  'year-header.theme-light':  'Light',
  'year-header.theme-dark':   'Dark',
  'year-header.theme-system': 'System',
  'year-header.extract-markdown': 'Share Markdown',
  'year-header.share-year':       'Share to Telos',
  'year-header.change-year':        'Change year ({year})',
  'year-header.year-picker-heading': 'Jump to year',
  'year-header.jump-to-today':       'Today',
  'year-header.reflection':          'Reflection',
  'year-header.reflection-add':     'Add',
  'year-header.reflection-score':   '★ {score}',
  'year-header.reflection-saved':   'Reflection saved',

  'reflection.dialog-heading':      'Year in review',
  'reflection.highlights-label':       'Highlights',
  'reflection.highlights-placeholder': 'What stood out this year…',
  'reflection.star-aria':           '{n} of 5 stars',
  'reflection.close':               'Close',
  'reflection.show':                'Show',
  'reflection.hide':                'Hide',
  'reflection.aspect-people-label':       'People',
  'reflection.aspect-people-hint':        'Family, friends, romance, social',
  'reflection.aspect-people-abbr':        'Pe',
  'reflection.aspect-health-label':       'Health',
  'reflection.aspect-health-hint':        'Physical, mental, emotional, environmental, spiritual',
  'reflection.aspect-health-abbr':        'He',
  'reflection.aspect-wealth-label':       'Wealth',
  'reflection.aspect-wealth-hint':        'Current, coming, planned, security, freedom',
  'reflection.aspect-wealth-abbr':        'We',
  'reflection.aspect-contribution-label': 'Contribution',
  'reflection.aspect-contribution-hint':  'Work, volunteering, making, sponsoring, teaching, innovating',
  'reflection.aspect-contribution-abbr':  'Co',
  'reflection.aspect-wonder-label':       'Wonder',
  'reflection.aspect-wonder-hint':        'Fun, exploration, creativity, adventure, education, interests',
  'reflection.aspect-wonder-abbr':        'Wo',
  'reflection.card-aspect-summary':       '{label} {value}',

  'not-found.heading': 'Nothing here',
  'not-found.body':    "This page doesn't exist — but your goals are waiting.",
  'not-found.cta':     'Go to this year',

  'sync.export-all':  'Export data',
  'sync.import':      'Import',
  'sync.import-preview':         'This file has {description}.',
  'sync.import-preview-empty':   'This file has no goals or list items.',
  'sync.import-preview-log':     'This file has {count} events.',
  'sync.preview-goals-one':      '{n} goal ({years})',
  'sync.preview-goals-many':     '{n} goals ({years})',
  'sync.preview-lists-one':      '{n} list with {items}',
  'sync.preview-lists-many':     '{n} lists with {items}',
  'sync.preview-items-one':      '{n} item',
  'sync.preview-items-many':     '{n} items',
  'sync.preview-and':            ' and ',
  'sync.import-cancel':          'Cancel',
  'sync.import-merge':           'Merge',
  'sync.import-replace':         'Replace all',
  'sync.import-sure':            'Sure?',
  'sync.import-sure-aria':       'Confirm: replace all data',
  'sync.import-confirm':         'Added {goals} goals and {items} list items.',
  'sync.import-replace-confirm': 'All data replaced.',
  'sync.import-close':           'Close',
  'sync.import-error':           'Invalid or incompatible export file.',
  'sync.handoff-goal-year-label':    'Year',
  'sync.handoff-goal-section-label': 'Section',
  'sync.handoff-goal-message':       '"{title}" was shared with you. Choose where to add it:',
  'sync.handoff-goal-confirm':       'Added "{title}".',
  'sync.handoff-item-confirm':       'Added "{title}".',
  'sync.handoff-items-confirm':      'Added {count} items.',
  'sync.handoff-add-to-list':        'Add to list',

  'share.error': "Couldn't share. Try again.",

  'lists-page.heading':   'Lists',
  'lists-page.add':       'New list',
  'lists-page.drag':     'Drag to reorder',
  'lists-page.menu':           'Menu',
  'lists-page.rollup-toggle':  'Date indicators',

  'list-detail.back':        'Lists',
  'list-detail.add':         'New item',
  'list-detail.menu':        'Options',
  'list-detail.edit-name':   'Edit list name',
  'list-detail.delete-list':         'Delete list',
  'list-detail.status-label': 'Status',
  'list-detail.status-show':        'Show',
  'list-detail.status-hide':        'Hide',
  'list-detail.cancel-selection':   'Cancel',
  'list-detail.selection-count':    '{n} items',
  'list-detail.bulk-delete':        'Delete',
  'list-detail.bulk-status':        'Status',
  'list-detail.bulk-move':          'Move',
  'list-detail.bulk-clear-dates':   'Clear due dates',
  'list-detail.bulk-tags':          'Tags',
  'list-detail.bulk-more':          'More',
  'list-detail.bulk-status-label':  'Set Status',
  'list-detail.bulk-tags-label':    'Add Tags',
  'list-detail.bulk-tag-apply-all': 'Apply {tag} to all',
  'list-detail.extract-markdown':      'Share Markdown',
  'list-detail.share-list':            'Share to Telos',
  'list-detail.archive-btn':             'Archive',
  'list-detail.unarchive-btn':           'Unarchive',
  'list-detail.bulk-extract-markdown': 'Share Markdown',
  'list-detail.bulk-share':            'Share to Telos',
  'list-detail.bulk-delete-toast':  'Deleted {n} items',
  'list-detail.bulk-clear-dates-toast': 'Cleared due date from {n} items',
  'list-detail.bulk-status-toast':  'Updated {n} items',

  'list-dialog.name-placeholder': 'List name',
  'list-dialog.close':            'Close',
  'list-dialog.save-and-close':   'Save and close',
  'list-dialog.title-new':        'New list',
  'list-dialog.title-edit':       'Edit list',
  'list-dialog.delete':           'Delete',
  'list-dialog.draft-clear':      'Clear',
  'list-dialog.draft-revert':     'Revert',
  'list-dialog.draft-undo':       'Undo',
  'list-dialog.draft-restore':    'Restore draft',

  'item-dialog.title-placeholder':'Title',
  'item-dialog.status-label':     'Status',
  'item-dialog.status-open':      'Open',
  'item-dialog.status-paused':    'Paused',
  'item-dialog.status-done':      'Done',
  'item-dialog.status-closed':    'Closed',
  'item-dialog.note-placeholder': 'Note',
  'item-dialog.copy-note': 'Copy note',
  'item-dialog.url-placeholder':  'Link (https://…)',
  'item-dialog.url-toggle':       'Link',
  'item-dialog.url-open':         'Open',
  'item-dialog.duedate-toggle':   'Due date',
  'item-dialog.duedate-clear':    'Clear due date',
  'item-dialog.close':            'Close',
  'item-dialog.save-and-close':   'Save and close',
  'item-dialog.title-new':        'New item',
  'item-dialog.title-edit':       'Edit item',
  'item-dialog.delete':           'Delete',
  'item-dialog.draft-clear':      'Clear',
  'item-dialog.draft-revert':     'Revert',
  'item-dialog.draft-undo':       'Undo',
  'item-dialog.draft-restore':    'Restore draft',
  'dialog.saved-sr':              'Saved',

  'item-dialog.move-to-list':           'Move to list',
  'item-dialog.add-to-goal':            'Add to goal',
  'item-dialog.extract-markdown':       'Share Markdown',
  'item-dialog.share-item':             'Share to Telos',
  'item-dialog.picker-back':            'Back',
  'item-dialog.move-cta':               'Move',
  'item-dialog.copy-cta':               'Copy',
  'item-dialog.no-other-lists':         'No other lists',
  'item-dialog.new-list-btn':           '＋ New list',
  'item-dialog.new-list-placeholder':   'List name',
  'item-dialog.goal-year-label':        'Year',
  'item-dialog.goal-section-label':     'Section',
  'item-dialog.goal-section-capstone':  'Capstone',
  'item-dialog.goal-section-milestones':'Milestones',
  'item-dialog.goal-section-wow':       'Wow Moments',
  'item-dialog.goal-section-focus':     'Focus',
  'item-dialog.goal-add-cta':           'Add to goal',
  'item-dialog.goal-already-added':     'Already added',
  'item-dialog.in-goals-label':         'In goals',
  'item-dialog.move-toast':             'Moved to {name}',
  'item-dialog.move-toast-many':        'Moved to {n} lists',
  'item-dialog.copy-toast':             'Copied to {name}',
  'item-dialog.copy-toast-many':        'Copied to {n} lists',
  'item-dialog.promote-toast':          'Added to {year} {section}',
  'item-dialog.tags-label':             'Tags',
  'item-dialog.tags-placeholder':       'Add tag…',

  'item-dialog.more-actions':           'More actions',
  'item-dialog.picker-selected':        '{n} selected',
  'item-dialog.picker-clear':           'Clear selection',

  'list-item.delete':          'Delete',
  'list-item.drag':      'Drag to reorder',
  'list-item.duedate-aria': '{title}, {when}',

  'lists.toast-list-saved':      'List saved',
  'lists.toast-list-hidden':     'List added — hidden by the current filter',
  'lists.toast-list-deleted':    '“{name}” deleted',
  'lists.toast-list-archived':   'List archived',
  'lists.toast-list-unarchived': 'List unarchived',
  'lists.toast-item-saved':   'Item saved',
  'lists.toast-item-hidden':  'Item added — hidden by the current filter',
  'lists.toast-item-deleted': 'Item deleted',

  'list-detail.filter-toggle':        'Filter',
  'list-detail.filter-search':        'Search items…',
  'list-detail.filter-status-all':    'All',
  'list-detail.filter-clear':         'Clear filters',
  'list-detail.filter-expand':        'More filters',
  'list-detail.filter-empty':         'No items match',
  'list-detail.filter-count':         '{count} items match',

  'lists-page.filter-toggle':         'Filter',
  'lists-page.filter-search':         'Search lists…',
  'lists-page.filter-expand':         'More filters',
  'lists-page.filter-clear':          'Clear filters',
  'lists-page.filter-empty':          'No lists match',
  'lists-page.filter-empty-only':     'Empty',
  'lists-page.filter-not-empty':      'Not empty',
  'lists-page.filter-archived':       'Archived',
  'lists-page.filter-count':          '{count} lists match',

  'home-page.filter-toggle':          'Filter',
  'home-page.filter-search':          'Search goals…',
  'home-page.filter-expand':          'More filters',
  'home-page.filter-done':            'Done',
  'home-page.filter-ongoing':         'Ongoing',
  'home-page.filter-not-started':     'Not started',
  'home-page.filter-archived':        'Archived',
  'home-page.filter-clear':           'Clear filters',
  'home-page.filter-empty':           'No goals match',
  'home-page.filter-count':           '{count} goals match',

  'list-detail.add-from-text':       'Add from text',
  'list-detail.import-toast':        'Imported {n} items',

  'import-text.heading':             'Import items',
  'import-text.placeholder':         'One item per line. Indent a line to make it a note.',
  'import-text.count':               '{n} items',
  'import-text.count-extras':        '{n} items · {m} with notes or links',
  'import-text.cancel':              'Cancel',
  'import-text.cta':                 'Import',
  'import-text.draft-clear':         'Clear',
  'import-text.draft-undo':          'Undo',

  'bottom-nav.years':    'Years',
  'bottom-nav.lists':    'Lists',
  'bottom-nav.settings': 'Settings',

  'settings.repair':            'Repair installation',
  'settings.export-reminder':   'Export reminder',
  'settings.reminder-on':       'Show',
  'settings.reminder-off':      'Hide',
  'settings.tag-strip':         'Tag colours',
  'settings.deadlines':         'Deadline markers',
  'settings.toggle-on':         'On',
  'settings.toggle-off':        'Off',
  'settings.level-off':         'Off',
  'settings.level-warn':        'Warn',
  'settings.level-full':        'Full',
  'settings.tags-shown':        'Shown',
  'settings.tags-hidden':       'Hidden',
  'settings.notifications':          'Notifications',
  'settings.notifications-on':       'On',
  'settings.notifications-off':      'Off',
  'settings.notifications-denied':   'Notifications are blocked for this site — enable them in your browser settings to turn this on.',
  'settings.notify-after':           'Notify after',
  'settings.notify-anytime':         'Any time',
  'settings.notifications-hint':     'Notifications require the app to be open.',

  'notifications.digest-title':      '{count} items need attention',
  'notifications.digest-overdue':    'Overdue ({count})',
  'notifications.digest-today':      'Due today ({count})',
  'notifications.digest-hidden':     '{count} hidden',
  'notifications.digest-hidden-only-title': '{count} hidden',
  'notifications.digest-hidden-only-body':  'Tap to review',
  'sync.backup-downloaded':     'Your data has been backed up before the update.',
  'sync.backup-before-import':  'Your data has been backed up.',

  'export-sheet.metadata': 'Metadata',
  'export-sheet.notes':    'Notes',
  'export-sheet.reflection': 'Reflection',
  'export-sheet.extract':  'Share Markdown',
  'export.copied':         'Copied to clipboard',

  'undo.button':  'Undo',
  'filter.toast-show': 'Show',
  'toast.close':  'Close',
  'tag.remove':   'Remove {tag}',

  // Shared colour-swatch picker (list-dialog, item-dialog, goal-dialog) — one
  // namespace since all three render the same COLOR_PALETTE from
  // app/utils/color-palette.js.
  'color-picker.none':   'No colour',
  'color-picker.red':    'Red',
  'color-picker.orange': 'Orange',
  'color-picker.yellow': 'Yellow',
  'color-picker.green':  'Green',
  'color-picker.teal':   'Teal',
  'color-picker.blue':   'Blue',
  'color-picker.purple': 'Purple',
});
